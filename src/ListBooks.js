...
//CurrentlyReading Bookshelf Functions
  filterCurrentlyReading = (book) => {
  ...
  fromCurrentlyReadingtoWantToRead = (book) => {
    this.filterCurrentlyReading(book)
    this.setState((currentState) => ({
      wantToRead: currentState.wantToRead.concat([book])
    }))
  }

  fromCurrentlyReadingtoRead = (book) => {
    this.filterCurrentlyReading(book)
    this.setState((currentState) => ({
...
// WantToRead Bookshelf Functions
  filterWantToRead = (book) => {
    this.setState((currentState) => ({
  ...
  fromWantToReadtoCurrentlyReading = (book) => {
    this.filterWantToRead(book)
    this.setState((currentState) => ({
      currentlyReading: currentState.currentlyReading.concat([book])
    }))
  }

  fromWantToReadtoRead = (book) => {
    this.filterWantToRead(book)
    this.setState((currentState) => ({
      read: currentState.read.concat([book])
    }))
...
//Read Bookshelf Functions
  ...
  fromReadtoWantToRead = (book) => {
    this.filterRead(book)
    this.setState((currentState) => ({
      wantToRead: currentState.wantToRead.concat([book])
    }))
  }

  fromReadtoCurrentlyReading = (book) => {
    this.filterRead(book)
    this.setState((currentState) => ({
      currentlyReading: currentState.currentlyReading.concat([book])
    }))
    ...
    return (
      <div className="list-books">
        ...
        <div className="list-books-content">
          <div>
            <CurrentlyReading bookList={this.state.currentlyReading}
                              moveToWant={this.fromCurrentlyReadingtoWantToRead}
                              moveToRead={this.fromCurrentlyReadingtoRead}/>
            <WantToRead bookList={this.state.wantToRead}
                        moveToCurrent={this.fromWantToReadtoCurrentlyReading}
                        moveToRead={this.fromWantToReadtoRead}/>
            <Read bookList={this.state.read}
                  moveToWant={this.fromReadtoWantToRead}
                  moveToCurrent={this.fromReadtoCurrentlyReading}/>
          </div>
        </div>
